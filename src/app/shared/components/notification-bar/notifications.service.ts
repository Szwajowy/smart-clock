...
import { bufferTime, last, debounceTime, buffer, takeLast, throttleTime } from 'rxjs/operators';
  ...
  subscribeToAll() {
    ...
    .subscribe(notificationsList => {
      ...
      notificationsList.forEach(notification => {
        indexOfExistingNot = finalNotificationsList.findIndex((finalNotification) => finalNotification.type === notification.type );
        ...
        if(res.operation === 'post') {
          ...
          this.notifications.forEach((notification, index) => {
            if(notification.type === res.type) {
              this.notifications[index] = { type: res.type, icon: res.icon, message: res.content };
              notificationExist = true;
            }
          });
    
          if(!notificationExist)
            this.notifications.push({ type: res.type, icon: res.icon, message: res.content });
