...
export class TimeCounter extends Time {
  ...
  increment(part: string, options?:{limitHours?:boolean, infinityScroll?:boolean}):boolean {
    ...
    switch(part) {
      ...
      case 'seconds':
        if(this[part] < 59) {
          ...
          if(!options || 
            !options.infinityScroll) {
            if(this.increment(part === 'minutes' ? 'hours' : 'minutes') === false)
              return false;
          }
    ...
    switch(part) {
        ...
        if(this[part] > 0) {
          ...
          if(!options || !options.infinityScroll || options.infinityScroll === false) {
            if(part === "milliseconds") {
              if(this.decrement('seconds') === false)
                return false;
            } else {
              return false;
            }
      ...
      case 'seconds':
        if(this[part] > 0) {
          ...
          if(!options || !options.infinityScroll || options.infinityScroll === false) {
            if(this.decrement(part === 'minutes' ? 'hours' : 'minutes') === false)
              return false;
          }
